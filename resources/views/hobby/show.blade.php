...
@section('title','hobby content detail')

@section('header')
    <h3 class="ml-5">{{"No.".$hobby->id}}</h3>
    <ul class="nav justify-content-end mr-5">
        <li class="nav-item">
            <a class="nav-link active" href="{{url('usertop')}}">Top page</a>
        </li>
        <li class="nav-item">
            <a class="nav-link active" href="{{action('HobbyController@index')}}">Back to index</a>
        </li>
    </ul>
@endsection
...
@section('maincontent')
    <div class="btn-group mb-3" role="group">
        <form method="POST" action="{{action('HobbyController@edit',['hobby'=>$hobby])}}">
            @csrf 
            @method('GET')
            <button class="btn btn-default mr-2">Edit</button>
        </form>
        <form method="POST" action="{{action('HobbyController@destroy',['hobby'=>$hobby])}}">
            @csrf 
            @method('DELETE')
    ...
    <div class="card ">
        ...
        <div class="card-header">
            <span class="text-left"><h4>{{$hobby->title}}</h4></span>
            <div class="text-right">
                <span class=" mr-2">{{"last modified　".$hobby->updated_at->format('Y年m月d日H時i分')}}</span>
                <span >{{"upload　".$hobby->created_at->format('Y年m月d日H時i分')}}</span>
            </div> 
        </div>
...
@section('subcontent')
    <div class="accordion col-7" id="accordionExample">
        @if($hobby->hobby_memos()->select('content')->get() != null)
            @foreach($hobby->hobby_memos()->select('id','content')->get() as $memo)
                <div class="card ">
...
@section('create_subcontent')
    <div class="card col-4 ml-3 px-0">
        <div class="card-body text-center px-1 py-1 ">
            <form method="POST" class="px-0 pt-0" action="{{action('HobbyMemoController@store')}}">
                @csrf 
                <input name="hobby_id" type="hidden" value="{{$hobby->id}}">
