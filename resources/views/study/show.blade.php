...
@section('title','study content detail')

@section('header')
    <h3 class="ml-5">{{"No.".$study->id}}</h3>
    <ul class="nav justify-content-end mr-5">
        <li class="nav-item">
            <a class="nav-link active" href="{{url('usertop')}}">Top page</a>
        </li>
        <li class="nav-item">
            <a class="nav-link active" href="{{action('StudyController@index')}}">Back to index</a>
        </li>
    </ul>
@endsection
...
@section('maincontent')
    ...
    <form method="POST" action="{{action('StudyController@edit',['study'=>$study])}}">
        @csrf 
        @method('GET')
        <button class="btn btn-default mr-2">Edit</button>
    </form>
    <form method="POST" action="{{action('StudyController@destroy',['study'=>$study])}}">
        @csrf 
        @method('DELETE')
    ...
    <div class="card ">
        <div class="card-header">
            <span class="text-left"><h4>{{$study->title}}</h4></span>
            <div class="text-right">
                <span class=" mr-2">{{"last modified　".$study->updated_at->format('Y年m月d日H時i分')}}</span>
                <span >{{"upload　".$study->created_at->format('Y年m月d日H時i分')}}</span>
            </div> 
        </div>
...
@section('subcontent')
    <div class="accordion col-7" id="accordionExample">
        @if($study->study_memos()->select('content')->get() != null)
            @foreach($study->study_memos()->select('id','content')->get() as $memo)
                <div class="card ">
...
@section('create_subcontent')
    <div class="card col-4 ml-3 px-0">
        <div class="card-body text-center px-1 py-1 ">
            <form method="POST" class="px-0 pt-0" action="{{action('StudyMemoController@store')}}">
                @csrf 
